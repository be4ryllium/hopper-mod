...
label0:
    ...
    public boolean blockActivated(World world, int i, int j, int k, EntityPlayer entityplayer)
    {
        ...
        if(!world.isBlockChestOpenable(i, j + 1, k))
        {
            return true;
        }
        if(world.getBlockId(i - 1, j, k) == blockID && !world.isBlockChestOpenable(i - 1, j + 1, k))
        {
            return true;
        }
        if(world.getBlockId(i + 1, j, k) == blockID && !world.isBlockChestOpenable(i + 1, j + 1, k))
        ...
        if(world.getBlockId(i, j, k - 1) == blockID && !world.isBlockChestOpenable(i, j + 1, k - 1))
        {
            return true;
        }
        if(world.getBlockId(i, j, k + 1) == blockID && !world.isBlockChestOpenable(i, j + 1, k + 1))
