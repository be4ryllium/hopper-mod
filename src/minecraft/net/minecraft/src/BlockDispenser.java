...
public class BlockDispenser extends BlockContainer
{
    ...
    private void dispenseItem(World world, int i, int j, int k, Random random)
    {
        ...
        } else
        {
            ...
            {
                tileentitydispenser.decrTheStack();
            ...
            if (itemstack.itemID == Item.bucketEmpty.shiftedIndex) {
                Material mat = world.getBlockMaterial(i + i1, j, k + j1);
                if ((mat == Material.water || mat == Material.lava) && world.getBlockMetadata(i + i1, j, k + j1) == 0) {
                    world.setBlockWithNotify(i + i1, j, k + j1, 0);

                    if (mat == Material.water) {
                        tileentitydispenser.setTheStackTo(new ItemStack(Item.bucketWater, 1));
                    } else {
                        tileentitydispenser.setTheStackTo(new ItemStack(Item.bucketLava, 1));
                    }

                    world.func_28106_e(1000, i, j, k, 0);
                } else {
                    world.func_28106_e(1001, i, j, k, 0);
                    tileentitydispenser.doNothingWithTheStack();
                }
            } else if (itemstack.itemID == Item.bucketWater.shiftedIndex) {
                if (world.isAirBlock(i + i1, j, k + j1) || !world.getBlockMaterial(i + i1, j, k + j1).isSolid()) {
                    if(world.worldProvider.isHellWorld)
                    {
                        world.playSoundEffect((double)(i + i1), (double)j, (double)(k + j1), "random.fizz", 0.5F, 2.6F + (world.rand.nextFloat() - world.rand.nextFloat()) * 0.8F);
                        for(int u = 0; u < 8; u++)
                        {
                            world.spawnParticle("largesmoke", (double)(i + i1) + Math.random(), (double)j + Math.random(), (double)(k + j1) + Math.random(), 0.0D, 0.0D, 0.0D);
                        }

                    } else {
                        world.setBlockAndMetadataWithNotify(i + i1, j, k + j1, Block.waterMoving.blockID, 0);
                        world.func_28106_e(1000, i, j, k, 0);
                    }
                    tileentitydispenser.setTheStackTo(new ItemStack(Item.bucketEmpty, 1));
                } else {
                    world.func_28106_e(1001, i, j, k, 0);
                    tileentitydispenser.doNothingWithTheStack();
                }
            } else if (itemstack.itemID == Item.bucketLava.shiftedIndex) {
                if (world.isAirBlock(i + i1, j, k + j1) || !world.getBlockMaterial(i + i1, j, k + j1).isSolid()) {
                    world.setBlockAndMetadataWithNotify(i + i1, j, k + j1, Block.lavaMoving.blockID, 0);
                    world.func_28106_e(1000, i, j, k, 0);
                    tileentitydispenser.setTheStackTo(new ItemStack(Item.bucketEmpty, 1));
                } else {
                    world.func_28106_e(1001, i, j, k, 0);
