    ...
    implements IBlockAccess
{
    ...
    public World(ISaveHandler isavehandler, String s, WorldProvider worldprovider, long l)
    {
        ...
        hopperClock = 0;
        if (worldInfo.getRawHopperList() == null) {
            worldInfo.initHopperList();
        }
    ...
    public World(ISaveHandler isavehandler, String s, long l, WorldProvider worldprovider)
    {
        ...
        hopperClock = 0;
        if (!isNewWorld && worldInfo.getRawHopperList() == null) {
            worldInfo.initHopperList();
        }
    ...
    public void addHopper(int i, int j, int k) {
        worldInfo.addToHopperList(new NextTickListEntry(i, j, k, Block.hopper.blockID));
    }

    public void removeHopper(int i, int j, int k) {
        for (int u = 0; u < worldInfo.getHopperListSize(); u++) {
            NextTickListEntry nextTickListEntry = (NextTickListEntry)worldInfo.getFromHopperList(u);
            if (nextTickListEntry.xCoord == i && nextTickListEntry.yCoord == j && nextTickListEntry.zCoord == k) {
                worldInfo.removeFromHopperList(u);
                return;
            }
        }
    }
    ...
    public boolean isBlockChestOpenable(int i, int j, int k) {
        Block block = Block.blocksList[getBlockId(i, j, k)];
        if (block == null) {
            return true;
        } else {
            return block.chestOpenable() || !isBlockNormalCube(i, j, k);
        }
    }
    ...
    public void tick()
    {
        ...
        if (hopperClock % 8 == 0) {
            ...
            for (int u = 0; u < worldInfo.getHopperListSize(); u++) {
                NextTickListEntry nextticklistentry = (NextTickListEntry)worldInfo.getFromHopperList(u);
                if (checkChunksExist(nextticklistentry.xCoord - byte0, nextticklistentry.yCoord - byte0, nextticklistentry.zCoord - byte0, nextticklistentry.xCoord + byte0, nextticklistentry.yCoord + byte0, nextticklistentry.zCoord + byte0)) {
                    int id = getBlockId(nextticklistentry.xCoord, nextticklistentry.yCoord, nextticklistentry.zCoord);
                    //System.out.println(String.valueOf(id));
                    if (id == Block.hopper.blockID) {
                        Block.hopper.updateItemFlow(this, nextticklistentry.xCoord, nextticklistentry.yCoord, nextticklistentry.zCoord);
                    }// else {
                        //removeHopper(nextticklistentry.xCoord, nextticklistentry.yCoord, nextticklistentry.zCoord);
                    //}
                }
            }
        }
